...
def approve_request(access_request):
    ...
    new_task_policy_step_id = (
        api.get_task(_get_access_token(), access_request.id)
        .get("taskView")
        .get("task", {})
        .get("policy", {})
        .get("current", {})
...
def _prepare_context_objects(access_token, task_summary):
    ...
    _entitlement_api_response = api.get_entitlement(
        access_token, task_summary.get("app_id"), task_summary.get("app_entitlement_id")
    )
    _entitlement_members_api_response = api.get_entitlement_members(
        access_token, task_summary.get("app_id"), task_summary.get("app_entitlement_id")
    )
    ...
    _user_api_response = api.get_user(
