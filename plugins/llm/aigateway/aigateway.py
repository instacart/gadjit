...
def query(system_prompt, user_prompt):
    ...
    try:
        ...
        logging.exception(f"The following content caused the JSONDecodeError: {response.content}")
        raise e
    ...
    try:
        ...
        logging.exception(f"OpenAI response could not be understood. The API returned the following content: {response.content}")
        raise e
...
def _get_access_token():
    ...
    if not AI_GATEWAY_ROLE_CREDENTIALS or (datetime.now() - AI_GATEWAY_ROLE_CREDENTIALS_TIMESTAMP > timedelta(minutes=5)):
        logging.debug(f"Refreshing AWS credentials. Last cached timestamp: {AI_GATEWAY_ROLE_CREDENTIALS_TIMESTAMP}")
        ...
        AI_GATEWAY_ROLE_CREDENTIALS = boto3_session.get_credentials().get_frozen_credentials()
        AI_GATEWAY_ROLE_CREDENTIALS_TIMESTAMP = datetime.now()
    else: 
        logging.debug(f"Using cached AWS credentials. Last cached timestamp: {AI_GATEWAY_ROLE_CREDENTIALS_TIMESTAMP}")
...
def __assume_role(role_arn):
    ...
    assumed_role = sts_client.assume_role(
        RoleArn=role_arn,
        RoleSessionName="aigateway-session"
    )   
